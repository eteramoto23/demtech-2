...
female <- readHMDweb( CNTRY = "USA", 
            item = "fltper_5x1", 
            username = "eteramoto@wisc.edu", 
            password = "83D-9b7tQT@#F8L", 
            fixup = TRUE )|>
  filter(Year == 2004)|>
  select(Age,
         Lx_f = Lx,
...
both <- male2 |>
  ...
  mutate(L_p_f = pov_f * Lx_f,
         ...
         ex_p_f = (1/lx_f)*Tx_f,
         ex_p_m = (1/lx_m)*Tx_m,
         s_sq_f = (pov_f * (1-pov_f)/num_f),
         s_sq_m = (pov_m * (1-pov_m)/num_m),
         var_e_f = (1/(lx_f)^2)*(((Lx_f)^2)*s_sq_f),
         var_e_m = (1/(lx_m)^2)*(((Lx_m)^2)*s_sq_m),
    z = (ex_p_f - ex_p_m)/(sqrt(var_e_f + var_e_m)),
